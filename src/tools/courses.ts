...
import type { Course, Module, Lesson } from "../types.js";
...
Returns: Paginated list of courses with provider, difficulty, and duration.`,
    ...
    async ({ category, difficulty, provider, limit, offset, response_format }) => {
      ...
      const { data, count, error } = await queryTable<Course>("courses", {
        limit,
        offset,
        filters,
      ...
      return {
        content: makeTextContent(paginated, response_format, (d) => {
          ...
          if (!p.items.length) return "No courses found matching your filters.";
          const lines = [
...
Returns: Course details with full module and lesson curriculum.`,
    ...
    async ({ course_id, response_format }) => {
      const { data: course, error: courseErr } = await getById<Course>("courses", course_id);
      if (courseErr || !course) {
        return {
          isError: true,
      ...
      const { data: modules } = await queryTable<Module>("modules", {
        filters: { course_id },
        order: { column: "order_index" },
        limit: 100,
      });

      const { data: lessons } = await queryTable<Lesson>("lessons", {
        filters: { course_id },
        order: { column: "order_index" },
        limit: 500,
      });
      ...
      return {
        content: makeTextContent(result, response_format, (d) => {
          const { course: c, modules: mods, lessons: ls } = d as typeof result;
...
Returns: Matching courses.`,
    ...
    async ({ query, limit, response_format }) => {
      ...
      return {
        content: makeTextContent(paginated, response_format, (d) => {
          const p = d as typeof paginated;
          if (!p.items.length) return `No courses found matching "${query}".`;
          return [
            `## Search Results for "${query}" (${p.total} found)\n`,
            ...p.items.map(
              (c) =>
                `**${c.title}** — ${difficultyBadge(c.difficulty)} | ${c.provider}\n` +
