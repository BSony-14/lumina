...
import type { User, CourseEnrollment, LessonProgress, Course, Certificate } from "../types.js";
...
Returns: User profile with role (student/mentor/admin).`,
    ...
    async ({ user_id, response_format }) => {
      const { data: user, error } = await getById<User>("users", user_id);
      if (error || !user) {
...
Returns: Paginated enrolled courses with progress.`,
    ...
    async ({ user_id, completed_only, limit, offset, response_format }) => {
      const filters: Record<string, string | number | boolean | null> = { user_id };
      if (completed_only) filters["completed"] = true;

      const { data: enrollments, count, error } = await queryTable<CourseEnrollment>("course_enrollments", {
      ...
      return {
        content: makeTextContent(paginated, response_format, (d) => {
          const p = d as typeof paginated;
          if (!p.items.length) return "No enrolled courses found.";
...
Returns: Lessons with completion status and time spent.`,
    ...
    async ({ user_id, course_id, response_format }) => {
      const { data, error } = await queryTable<LessonProgress>("lesson_progress", {
...
Returns: Confirmation with updated progress percent.`,
    ...
    async ({ user_id, lesson_id, course_id, time_spent_minutes }) => {
      ...
      const { error: upsertErr } = await supabase.from("lesson_progress").upsert(
        { user_id, lesson_id, course_id, completed: true, completed_at: new Date().toISOString(), time_spent_minutes },
        { onConflict: "user_id,lesson_id" }
      );
      if (upsertErr) return { isError: true, content: [{ type: "text", text: `Failed: ${upsertErr.message}` }] };
...
Returns: Certificates with course info and issue date.`,
    ...
    async ({ user_id, response_format }) => {
      const { data: certs, error } = await queryTable<Certificate>("certificates", {
      ...
      return {
        content: makeTextContent(certs, response_format, (d) => {
          const cs = d as Certificate[];
          if (!cs.length) return "No certificates earned yet.";
          return [`## Certificates (${cs.length} earned)\n`, ...cs.map((c) =>
            `- 🏆 Course \`${c.course_id}\` — **${formatDate(c.issued_at)}**` + (c.certificate_url ? ` | [View](${c.certificate_url})` : "")
          )].join("\n");
        }),
