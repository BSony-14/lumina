...
export function registerMentorTools(server: McpServer): void {
  server.registerTool(
    "lms_list_mentors",
    {
...
Returns: Paginated list of mentor profiles.`,
    ...
    async ({ limit, offset, response_format }) => {
      const { data, count, error } = await queryTable<User>("users", {
      ...
      return {
        content: makeTextContent(paginated, response_format, (d) => {
          ...
          if (!p.items.length) return "No mentors found.";
          return [`## Mentors (${p.total} total)\n`, ...p.items.map((m) =>
            `- **${m.full_name}** — ${m.email} | Joined ${formatDate(m.created_at)} | ID: \`${m.id}\``
          )].join("\n");
        }),
...
Returns: Paginated list of sessions with Jitsi room info.`,
    ...
    async ({ course_id, mentor_id, status, limit, offset, response_format }) => {
      ...
      const { data, count, error } = await queryTable<LiveSession>("live_sessions", {
        filters, limit, offset, order: { column: "scheduled_at" },
      });
      if (error) return { isError: true, content: [{ type: "text", text: `Error: ${error}` }] };

      const paginated = formatPagination(data, count ?? data.length, offset, limit);
      const statusEmoji: Record<string, string> = { scheduled: "📅", live: "🔴", completed: "✅", cancelled: "❌" };

      return {
        content: makeTextContent(paginated, response_format, (d) => {
          const p = d as typeof paginated;
          if (!p.items.length) return "No live sessions found.";
          return [`## Live Sessions (${p.total} total)\n`, ...p.items.map((s) =>
            `### ${statusEmoji[s.status] ?? ""} ${s.title}\n` +
...
Returns: Created session with Jitsi join link.`,
    ...
    async (params) => {
      ...
      const { data: session, error } = await insertRow<LiveSession>("live_sessions", {
...
Returns: Confirmation of update.`,
    ...
    async ({ session_id, status }) => {
      const { data, error } = await updateRow<LiveSession>("live_sessions", session_id, { status });
      if (error || !data) return { isError: true, content: [{ type: "text", text: `Failed: ${error}` }] };
