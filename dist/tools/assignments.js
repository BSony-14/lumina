...
export function registerAssignmentTools(server) {
    server.registerTool("lms_list_assignments", {
...
Returns: Assignments with due dates and max scores.`,
        ...
        const { data, count, error } = await queryTable("assignments", {
    ...
    server.registerTool("lms_submit_assignment", {
...
Returns: Confirmation with submission ID and timestamp.`,
        ...
        const { data: existing } = await supabase
            ...
            .maybeSingle();
        if (existing) {
            const alreadyResult = { already_submitted: true, submission_id: existing.id };
            return {
                content: [{ type: "text", text: `⚠️ Already submitted on ${formatDate(existing.submitted_at)}. ID: \`${existing.id}\`` }],
                structuredContent: toRecord(alreadyResult),
            };
        }
        const { data: submission, error } = await insertRow("assignment_submissions", {
            assignment_id, user_id, content, submitted_at: new Date().toISOString(),
        });
        if (error || !submission)
            return { isError: true, content: [{ type: "text", text: `Failed to submit: ${error}` }] };
        return {
            content: [{ type: "text", text: `✅ Submitted!\n- **ID**: \`${submission.id}\`\n- **At**: ${formatDate(submission.submitted_at)}\n\nYour mentor will review and grade it.` }],
            structuredContent: toRecord(submission),
        };
    });
    server.registerTool("lms_get_submission", {
...
Returns: Submission content, score, and mentor feedback.`,
        ...
        if (!data)
            return { content: [{ type: "text", text: "No submission found." }] };
    ...
    server.registerTool("lms_grade_assignment", {
...
Returns: Confirmation of grade recorded.`,
        ...
        const { data, error } = await updateRow("assignment_submissions", submission_id, {
            score, feedback, graded_at: new Date().toISOString(),
        });
        if (error || !data)
            return { isError: true, content: [{ type: "text", text: `Failed to grade: ${error}` }] };
        return {
